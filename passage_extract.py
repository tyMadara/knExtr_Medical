...
def eliminate2(infile,outfile):
    ...
    for ele in uList:
        if len(ele.name)>2:
            ...
            ignore_word=['学会','检查','技术','平面','时间','分数','显示','气体','固体','液体','目标','多发','一条','问题','现象'\
                         '体积','增大','大小','多少','能力','杂志','学会','不同','分类','相同','学说','功能',\
                         '出现','类型','一侧','第二','某种','严重','利用','十三','十二','十一','上述','十五','十六','液态']
            if ele.name not in ignore_word:
                resList.append(ele)
            #else:
            #    print(ele.name)
    ulisttxt_Writefromulist(resList, outfile)
...
def test(infile,inuList='',indict=''):
    sentences = sentence_Readfromdoc(infile)
    uList = ulist_Readfromtxt(inuList)
    indict=r'..\实体提取\dict.txt'
    jieba.load_userdict(indict)
    resList = regular_extract(sentences, uList, infile)
def eliminate3(ufile,infile,indict):
    sentences = sentence_Readfromdoc(infile)
    #uList = ulist_Readfromtxt(ufile)
    dict=dict_readformuList(ufile)
    jieba.load_userdict(indict)
    ofh = open(r'..\实体提取\1.csv', 'w',encoding='utf-8-sig', newline='')
    ...
    for sent in sentences:
        ...
        for i in range(len(posList)-1):
            if posList[i][1]=='n' and posList[i+1][1]=='n' and posList[i][0] in dict and posList[i+1][0] in dict:
                writer.writerow([posList[i][0],posList[i+1][0]])
def eliminate4(ufile):
    uList = ulist_Readfromtxt(ufile)
    for ele in uList:
        if '及'in ele.name or '的'in ele.name or len(ele.name)>10:
            print(ele.name)
